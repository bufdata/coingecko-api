...
// GetCirculatingSupplyChartByCoinID gets historical circulating supply of a coin, by number of days away from now.
//
...
// GetCirculatingSupplyChartRangeByCoinID gets historical circulating supply of a coin, within a range of timestamp.
//
// CoinGecko equivalent page: https://www.coingecko.com/en/exchanges/binance#statistics (Exchange Trade Volume chart)
//
// Data Granularity (auto): 1 day from now = 5-minutely data; 2-90 days from now = hourly data; 91 days & above
...
// ListAllTokensByAssetPlatformID gets full list of tokens of a blockchain network (asset platform) that is supported by Ethereum
...
func (c *Client) ListAllTokensByAssetPlatformID(ctx context.Context, assetPlatformID string) (*TokensListAllResponse, error) {
	...
	resp, _, err := c.sendReq(ctx, endpoint)
	if err != nil {
		slog.Error("failed to send request to list all tokens api", "error", err)
		return nil, err
	}
